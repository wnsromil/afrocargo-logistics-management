<x-app-layout>
    ...
    <form id="expenseFilterForm" action="{{ route('admin.end_of_day.Supply_index') }}" method="GET">
    ...
    <div class="fw-600 mb-3 fs-6 summary-text">
        ...
        BoxCredit: <span class="summary-amount">$0.00</span> &nbsp; | &nbsp;
        QuickPay: <span class="summary-amount">$0.00</span> &nbsp; | &nbsp;
        ManualCC: <span class="summary-amount">$0.00</span> &nbsp; | &nbsp;
        Deposit: <span class="summary-amount">$0.00</span>
    </div>
    <hr>

    <div class="fw-600 fs-6 d-flex flex-wrap gap-3 summary-text">
        <div>Total Cash: <span class="summary-amount">${{ number_format($totalCash, 2) }}</span></div>
        <div>Expenses: <span class="summary-amount">$0.00</span></div>
        <div>Total Inv. Amt: <span class="summary-amount">${{ number_format($totalInvoiced, 2) }}</span></div>
        <div>Total Balance: <span class="summary-amount">${{ number_format($totalBalance, 2) }}</span></div>
    </div>
    <hr>
                        ...
                        <tbody>
                            @forelse ($invoices as $key => $invoice)
                                <tr>
                                    ...
                                    <td>${{ number_format($invoice->payment ?? 0, 2) }}</td>
    ...
    @section("script")
        <script>
            ...
            function resetForm() {
                window.location.href = "{{ route('admin.end_of_day.Supply_index') }}";
            }
        </script>
        <script>
