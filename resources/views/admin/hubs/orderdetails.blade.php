            ...
            <div class="row mt-4 ">
                <div class="col-md-12">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="d-flex">
                            {{-- @if ($parcel->ParcelInventory->whereNull('invoice_id')->count() > 0)
                            <button type="button" class="btn btn-primary btnf me-2" id="addInvoiceBtn">
                                <i class="ti ti-circle-plus me-2 text-white"></i>Add Invoice
                            </button>
                            @endif --}}

                            @if ($parcel->invoice_id)
                                {{-- <button type="button" class="btn btn-primary btnf me-2" data-bs-toggle="modal"
                    ...
                    <div class="table-responsive notMinheight smallTDs">
                        <table class="table table-stripped table-hover datatable notposition">
                            ...
                            <tbody>
                                @forelse ($parcel->ParcelInventory as $index => $parcelItem)
                                    <tr>
                                        <td>
                                            <input type="checkbox" name="parcel_items[]" class="itemCheckbox"
                                                value='{{ $parcelItem->id }}'
                                                {{ $parcelItem->status == 11 ? 'checked disabled' : '' }}>
                                        </td>
                                        <td>{{$index + 1}}</td>
                                        ...
                                        <td>{{ucfirst($parcelItem->supply_name ?? '')}}</td>
                                        <td>{{$parcelItem->qty ?? "0"}}</td>
    ...
    @section('script')
            ...
            $(document).ready(function () {
                $('#addInvoiceBtn').on('click', function () {
                    ...
                    $('.itemCheckbox:checked').each(function () {
                        const itemData = JSON.parse(this.value);
                        // Yaha aapko full object milega
                        selectedItems.push(itemData);
                        totalAmount += parseFloat(itemData.total || 0);

                        // Add total quantity
                        totalQty += parseInt(itemData.qty || 0);
                    });

                    // ✅ If no checkbox is selected
                    if (selectedItems.length === 0) {
                        toastr["error"]("Please select at least one item to create an invoice.");
                        return;
                    }
                    // ✅ Example: Get values from Blade or hidden inputs
                    const parcel_type = "{{ $parcel->parcel_type ?? '' }}";
                    let invoice_type = "";
                    if (parcel_type == "Service") {
                        invoice_type = "services";
                    } else {
                        invoice_type = "supplies";
                    }

                    // ✅ Send AJAX POST
                    $.ajax({
                        url: '/invoice-create-order-details-service', // replace with your route
                        method: 'POST',
                        data: {
                            parcel_id: "{{ $parcel->id ?? '' }}",
                            invoce_type: invoice_type,
                            customer_id: "{{ $parcel->customer_id ?? '' }}",
                            ship_customer_id: "{{ $parcel->ship_customer_id ?? '' }}",
                            ...
                            total_qty: totalQty,
                            customer_subcategories_data: selectedItems,
                            duedaterange: 'nullable|string',
                            pickup_date: "{{ Carbon\Carbon::parse($parcel->pickup_date ?? '')->format('Y-m-d') }}",
       ...
       <script>
            document.getElementById('self_pickup_img').addEventListener('change', function (event) {
                ...
                // Correct id here
                document.getElementById('upload_self_pickup_img').style.display = 'none';
                }
            });
            </script>
    @endsection
